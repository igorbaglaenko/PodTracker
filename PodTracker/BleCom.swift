...
class BleCom : ObservableObject {
    ...
    func onReceiveBleMsg() {
        switch (bleState) {
        case .connecting:
            podData.onReceiveStatusData(status: "Connecting...")
            bleState = bleStates.services
            break
        case .services:
            podData.onReceiveStatusData(status: "Discovering POD Services")
            bleState = bleStates.conneceted
            break
        case .conneceted:
            podData.onReceiveStatusData(status: "Connected")
            bleState = bleStates.readPodId
            break
        case .readPodId:
            buildPodIdData()
            podData.onReceivePodID(rowBytes: podIdData)
            bleState = bleStates.readDailyData
            break
        case .readDailyData:
           // buildDailyData()
            podData.onReceiveDailyData(rowBytes: podDailyData)
            bleState = bleStates.readWeeklyData
            break
        case .readWeeklyData:
            txIndex = podData.onReceiveTotalData(rowBytes: buildWeeklyData(startIndx: txIndex), startIndex: txIndex)
            if txIndex == 0 {
        ...
        case .monitoring:
            ...
            podData.onReceiveMonitorData(rowBytes: data)
            break
